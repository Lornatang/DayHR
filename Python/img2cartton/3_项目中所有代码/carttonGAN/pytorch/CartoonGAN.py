...
parser.add_argument(
    '--src_data',
    required=False,
    default='src_data_path',
    help='sec data path')
parser.add_argument(
    '--tgt_data',
    required=False,
    default='tgt_data_path',
    help='tgt data path')
parser.add_argument(
    '--vgg_model',
    required=False,
...
parser.add_argument('--out_ngc', type=int, default=3,
                    help='output channel for generator')
parser.add_argument(
    '--in_ndc',
    type=int,
    default=3,
    help='input channel for discriminator')
parser.add_argument(
    '--out_ndc',
    type=int,
    default=1,
    help='output channel for discriminator')
...
parser.add_argument(
    ...
    type=int,
    default=8,
    help='the number of resnet block layer for generator')
...
train_loader_src = utils.data_load(
    os.path.join(
        'data',
        args.src_data),
    'train',
    ...
    shuffle=True,
    drop_last=True)
train_loader_tgt = utils.data_load(
    os.path.join(
        'data',
        args.tgt_data),
    'pair',
    ...
    shuffle=True,
    drop_last=True)
test_loader_src = utils.data_load(
    os.path.join(
        'data',
        args.src_data),
    'test',
...
if args.latest_discriminator_model != '':
    if torch.cuda.is_available():
        ...
        D.load_state_dict(
            torch.load(
                args.latest_discriminator_model,
                map_location=lambda storage,
                loc: storage))
...
if args.latest_generator_model == '':
    ...
    for epoch in range(args.pre_train_epoch):
        ...
        print(
            ...
            ((epoch +
             ...
             per_epoch_time,
             torch.mean(
                torch.FloatTensor(Recon_losses))))
    ...
    with torch.no_grad():
        ...
        for n, (x, _) in enumerate(train_loader_src):
            ...
            path = os.path.join(args.name + '_results',
                                'Reconstruction',
...
for epoch in range(args.train_epoch):
    ...
    print(
        ...
        ((epoch +
         ...
         per_epoch_time,
         torch.mean(
            torch.FloatTensor(Disc_losses)),
            torch.mean(
            torch.FloatTensor(Gen_losses)),
            torch.mean(
            torch.FloatTensor(Con_losses))))
        ...
        with torch.no_grad():
            ...
            for n, (x, _) in enumerate(train_loader_src):
                ...
                path = os.path.join(args.name +
                                    ...
                                    '.png')
                plt.imsave(
                    path, (result.cpu().numpy().transpose(
                        1, 2, 0) + 1) / 2)
                if n == 4:
                    break
...
print(
    "Avg one epoch time: %.2f, total %d epochs time: %.2f" %
    (torch.mean(
        torch.FloatTensor(
            train_hist['per_epoch_time'])),
     args.train_epoch,
